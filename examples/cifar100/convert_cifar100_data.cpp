...
const int kCIFARTrainBatchSize = 50000;
const int kCIFARTestBatchSize = 10000;
...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    ...
    for (int itemid = 0; itemid < kCIFARTrainBatchSize; ++itemid) {
      read_image(&data_file, &label, str_buffer);
      datum.set_label(label);
      datum.set_data(str_buffer, kCIFARImageNBytes);
      string out;
      CHECK(datum.SerializeToString(&out));
      txn->Put(caffe::format_int(fileid * kCIFARTrainBatchSize + itemid, 5),
               out);
    }
  }
  txn->Commit();
  ...
  for (int itemid = 0; itemid < kCIFARTestBatchSize; ++itemid) {
    read_image(&data_file, &label, str_buffer);
    datum.set_label(label);
    datum.set_data(str_buffer, kCIFARImageNBytes);
    string out;
