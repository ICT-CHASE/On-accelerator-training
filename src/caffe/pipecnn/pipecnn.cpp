...
#include "caffe/pipecnn/ocl_util.h"
#include "caffe/pipecnn/timer.h"
#include "caffe/pipecnn/pipecnn.h"

// CNN network configuration file
#include "caffe/pipecnn/hw_param.cl"
#include "caffe/pipecnn/layer_config.h"
...
const char *mean_data_file_path    = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/imagenet/mean_data.dat";
const char *synset_word_file_path  = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/imagenet/synset_words.txt";
const char *LabelPath		   = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/imagenet/val.txt";
...
#define LAYER_NUM         8
#define CONV_NUM          5
const char *weight_file_path = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/data_alex/weights.dat";
const char *input_file_path = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/data_alex/image.dat";
const char *ref_file_path = "/home/xingkouzi/workspace/pipecnn/src/PipeCNN/data/data_alex/fc8.dat";
...
const char *weight_file_path = "/home/xkz/workspace/pipecnn/src/PipeCNN/data/data_alex/weights.dat";
const char *input_file_path = "/home/xkz/workspace/pipecnn/src/PipeCNN/data/data_alex/image.dat";
const char *ref_file_path = "/home/xkz/workspace/pipecnn/src/PipeCNN/data/data_alex/fc8.dat";
const char *dump_file_path = "./result_dump.txt";

//const char *weight_file_path = "/home/xkz/workspace/pipecnn-5-17/src/PipeCNN/data/data_vgg/weights.dat";
...
#endif
void loadImageToBuffer(int num,signed char* imageCaffe);
int  prepare(signed char *weightsCaffe);
void readDataBack(int layer,signed char *outputCaffe);
...
//int pipeCNN(int count, char** param,signed char *weightsCaffe,signed char *imageCaffe,signed char *outputCaffe)
//{
int memoryCreat(){
	int count=2;
	cl_int status;
...
//xkz:bug
layer_config[0][data_n] =3;
layer_config[0][weight_n]=3;
	...
	checkError(status, "Failed to create context");
        ...
        //xkz: argv 替换位binary_container_1.xclbin
	// Create Program Objects
	char *kernel_file_name="/home/xingkouzi/caffe/caffe-pipecnn-alexnet/binary_container_1.xclbin";//param[1];
...
//std::cout<<"-1="<<layer_config[0][weight_n]<<std::endl;
	// Prepare compute data
//	status = prepare(weightsCaffe);
//	if(status == 1){
//		printf("Allocate memory for data and weights failed !!!\n");
//		return false;
//	}
//xkz:bug
layer_config[0][data_n] =4;
layer_config[0][weight_n]=4;


//std::cout<<"-0="<<layer_config[0][weight_n]<<std::endl;
...
//std::cout<<"0="<<layer_config[j][weight_n]<<std::endl;
		    weight_buf_size = layer_config[j][weight_w]*layer_config[j][weight_h]*layer_config[j][weight_n]*layer_config[j][weight_m];
//std::cout<<"1="<<layer_config[j][weight_n]<<std::endl;
//std::cout<<weight_buf_size<<std::endl;
...
#endif
		loadImageToBuffer(pic_num,imageCaffe);

		// Recorde the start time
		t.start();
int layerLoop[]={1,1,1,1,2,1,1,1};
		// Each iteration excutes one layer convolution
		// MemRd -> Conv(Relu) -> (MaxPool) -> MemWr -> (Lrn)
		for(unsigned char j = 0; j < LAYER_NUM; ++j){
//xkz:fetch conv data
//        int layerLoop[]={1,1,1,1,2,1,1,1};
        for(int tempLoop=0;tempLoop<layerLoop[j];tempLoop++){
                if(j==4&&tempLoop==0){
                        layer_config[4][pool_on]=0;
                }
                else if(j==4&&tempLoop==1){
                        layer_config[4][pool_on]=1;
                }
...
//std::cout<<"pipecnn.cpp 548:"<<layer_config[j][data_w]<<std::endl;
...
//cout<<"pipecnn.cpp 870:here"<<endl;
...
//cout<<"pipecnn.cpp 880:here"<<endl;
#ifndef USE_OPENCV
...
//	printf("Total kernel runtime %0.3f ms \n", double(kernel_time) * 1e-6);
...
void readDataBack(int layer,signed char *outputCaffe)
{
	...
	if(batch_item_num>(input_config[batch_size]-1)){
        ...
        //xkz:修改read_buf_size
	if(layer_config[layer][pool_on]){
        	read_buf_size=layer_config[layer][pool_x]*layer_config[layer][pool_y]*layer_config[layer][pool_z];
	}
	else{
		read_buf_size=layer_config[layer][conv_x]*layer_config[layer][conv_y]*layer_config[layer][conv_z];
	}
...
*/
	...
	if(layer_config[layer][memwr_dst] == 2){
//		printf("\nCopyed all batched results from fc_1 buffers.\n");
	...
	else if(layer_config[layer][memwr_dst] == 3){
//		printf("\nCopyed all batched results from fc_2 buffers.\n");
	...
	else if(layer_config[layer][memwr_dst]^layer_config[layer][lrn_on]){// if lrn is used, the mem dst is changed back to src
...
//		printf("\nCopyed one results from NO.%d data buffers.\n", batch_item_num);
	...
	//xkz:修改extractoutput的参数(其实都是pool的size,pool_on==0时，conv的size==pool的size)
...
//	}
//	else{
		if(layer_config[layer][pool_on]==1){
			extractOutput(output, output_one_item, 0, 1, layer_config[layer][pool_x], layer_config[layer][pool_y], layer_config[layer][pool_z]);
		}
		else{
			extractOutput(output, output_one_item, 0, 1, layer_config[layer][conv_x], layer_config[layer][conv_y], layer_config[layer][conv_z]);
		}

	//xkz:修改reorderoutput的参数
        //reorderOutput(output_one_item, output_reorder, output_config[output_w], output_config[output_h], output_config[output_n]);
	if(layer_config[layer][pool_on]==1){
		reorderOutput(output_one_item, output_reorder, layer_config[layer][pool_x], layer_config[layer][pool_y], layer_config[layer][pool_z]);
	}
	else{
		reorderOutput(output_one_item, output_reorder, layer_config[layer][conv_x], layer_config[layer][conv_y], layer_config[layer][conv_z]);
	}
        //xkz:reorder后的数据返回caffe
	static int caffev=0;
        int outputCountCaffe=read_buf_size;//=layer_config[layer][pool_x]*layer_config[layer][pool_y]*layer_config[layer][pool_z];
        for(int caffei=0;caffei<outputCountCaffe;caffei++){
	  outputCaffe[caffev]=output_reorder[caffei];
	  caffev++;
        }
        if(layer==7) caffev=0;
//std::cout<<"caffev="<<caffev<<std::endl;
}
...
*///xkzxkz
//xkz:image debug
/*for(int debugi=0;debugi<30;debugi++){
  std::cout<<"image fixed["<<debugi<<"]="<<image[debugi]+100<<std::endl;
}*/
		...
		if(ll==0){ // check parameters for layer-1
			if(input_config[image_w] != layer_config_original[ll][data_w] ||  input_config[image_h] != layer_config_original[ll][data_h]
				|| input_config[image_n] != layer_config_original[ll][data_n] || input_config[image_n] != layer_config_original[ll][weight_n]){
					...
					//return 1;
                                        //std::cout<<input_config[image_w]<<"  "<<input_config[image_h]<<"  "<<input_config[image_n]<<std::endl;
                                        //std::cout<<layer_config_original[ll][data_w]<<"  "<<layer_config_original[ll][data_h]<<"  "<<layer_config_original[ll][data_n]<<"  "<<layer_config_original[ll][weight_n]<<std::endl;
				}
...
//std::cout<<layer_config[0][weight_n]<<"   weight"<<std::endl;
...
//xkz:bug
//layer_config[0][data_n] = layer_config[0][weight_n]=3;
...
//xkz:caffe's weights to pipeCNN's weights
    weights=weightsCaffe;
//xkz:weight debug
/*    for(int xkz=61063552-1024-24*4096-15;xkz<61063552-1024-24*4096-15+30;xkz++){
    	std::cout<<"weights="<<weights[xkz]+100<<std::endl;
    }*/
