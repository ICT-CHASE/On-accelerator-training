...
#include "caffe/fixpoint/base_conv_float_layer.hpp"
...
void BaseConvolutionFloatLayer<Dtype>::LayerSetUp(
  ...
  // output_max_ = 1 << 30;
  CHECK_GT(output_max_, 0) << "output max should be greater than 0";
  ...
  LOG(INFO) << "qcode config for output: Q" << output_config_.il() << ","
            << output_config_.fl();
  if (bias_term_) {
    bias_config_.set_il(input_config_.il() + weight_config_.il());
    bias_config_.set_fl(input_config_.fl() + weight_config_.fl());
    LOG(INFO) << "qcode config for bias: Q" << bias_config_.il() << ","
              << bias_config_.fl();
  }
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(bottom.size(), 1) << "input blobs should be 1";
  const int first_spatial_axis = channel_axis_ + 1;
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
                                                        const Dtype* weights,
                                                        Dtype* output,
                                                        bool skip_im2col) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::forward_cpu_bias(Dtype* output,
                                                        const Dtype* bias) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
                                                         const Dtype* weights,
                                                         Dtype* input) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::weight_cpu_gemm(const Dtype* input,
                                                       const Dtype* output,
                                                       Dtype* weights) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::backward_cpu_bias(Dtype* bias,
                                                         const Dtype* input) {
...
template <>
void BaseConvolutionFloatLayer<double>::forward_gpu_gemm(const double* input,
...
template <>
void BaseConvolutionFloatLayer<float>::forward_gpu_gemm(const float* input,
                                                        const float* weights,
                                                        float* output,
                                                        bool skip_im2col) {
  ...
  for (int g = 0; g < group_; ++g) {
    // replace caffe_gpu_gemm by fixpoint version
    // A: weight
    // B: input
    // C: output
    gpu_gemm_fp(CblasNoTrans, CblasNoTrans, conv_out_channels_ / group_,
                conv_out_spatial_dim_, kernel_dim_, (float)1.,
                weights + weight_offset_ * g, col_buff + col_offset_ * g,
                (float)0., output + output_offset_ * g, output_max_);
  }
}

template <>
void BaseConvolutionFloatLayer<double>::forward_gpu_bias(double* output,
                                                         const double* bias) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::forward_gpu_bias(Dtype* output,
                                                        const Dtype* bias) {
  // caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
  //                       out_spatial_dim_, 1, (Dtype)1., bias,
  //                       bias_multiplier_.gpu_data(), (Dtype)1., output);

  gpu_gemm_fp(CblasNoTrans, CblasNoTrans, num_output_, out_spatial_dim_, 1,
              (float)1., bias, bias_multiplier_.gpu_data(), (float)1., output,
              output_max_);
}

template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
                                                         const Dtype* weights,
                                                         Dtype* input) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
                                                       const Dtype* output,
                                                       Dtype* weights) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
...
template <typename Dtype>
void BaseConvolutionFloatLayer<Dtype>::backward_gpu_bias(Dtype* bias,
                                                         const Dtype* input) {
...
INSTANTIATE_CLASS(BaseConvolutionFloatLayer);
