...
__global__ void MaxPoolForward(
    const int nthreads, const Dtype* const bottom_data, const int num,
...
__global__ void AvePoolForward(
    ...
    const int pooled_height, const int pooled_width, const int kernel_h,
    const int kernel_w, const int stride_h, const int stride_w, const int pad_h,
    const int pad_w, Dtype* const top_data, const Dtype scale_factor,
    const Dtype sum_max, const Dtype out_max) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    const int pw = index % pooled_width;
    const int ph = (index / pooled_width) % pooled_height;
    const int c = (index / pooled_width / pooled_height) % channels;
    ...
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        ...
        if (aveval > sum_max) aveval = sum_max;
        if (aveval < -sum_max) aveval = -sum_max;
      }
    }
    ...
    res = res * (1 << 4);
    int intres = round(res);
    res = Dtype(intres) / (1 << 4);
    top_data[index] = res;
  }
}
...
template <typename Dtype>
void PoolingFloatLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  switch (this->layer_param_.pooling_param().pool()) {
    ...
    case PoolingParameter_PoolMethod_AVE:
      ...
      AvePoolForward<Dtype>
          <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
              count, bottom_data, bottom[0]->num(), channels_, height_, width_,
              pooled_height_, pooled_width_, kernel_h_, kernel_w_, stride_h_,
              stride_w_, pad_h_, pad_w_, top_data, this->approx_, sum_max_,
              out_max_);
      break;
    case PoolingParameter_PoolMethod_STOCHASTIC:
      if (this->phase_ == TRAIN) {
        ...
        // NOLINT_NEXT_LINE(whitespace/operators)
        StoPoolForwardTrain<Dtype>
            <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
                count, bottom_data, bottom[0]->num(), channels_, height_,
                width_, pooled_height_, pooled_width_, kernel_h_, kernel_w_,
                stride_h_, stride_w_, rand_idx_.mutable_gpu_data(), top_data);
      } else {
        // NOLINT_NEXT_LINE(whitespace/operators)
        StoPoolForwardTest<Dtype>
            <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
                count, bottom_data, bottom[0]->num(), channels_, height_,
                width_, pooled_height_, pooled_width_, kernel_h_, kernel_w_,
...
__global__ void AvePoolBackward(
    const int nthreads, const Dtype* const top_diff, const int num,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, const int kernel_h,
    const int kernel_w, const int stride_h, const int stride_w, const int pad_h,
    const int pad_w, Dtype* const bottom_diff, const Dtype scale_factor) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    // find out the local index
    // find out the local offset
    ...
    for (int ph = phstart; ph < phend; ++ph) {
      for (int pw = pwstart; pw < pwend; ++pw) {
        ...
        gradient += top_diff_slice[ph * pooled_width + pw] * scale_factor;
      }
    }
    bottom_diff[index] = gradient;
  }
...
template <typename Dtype>
void PoolingFloatLayer<Dtype>::Backward_gpu(
  ...
  switch (this->layer_param_.pooling_param().pool()) {
    ...
    case PoolingParameter_PoolMethod_AVE:
      ...
      AvePoolBackward<Dtype>
          <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
              count, top_diff, top[0]->num(), channels_, height_, width_,
              pooled_height_, pooled_width_, kernel_h_, kernel_w_, stride_h_,
              stride_w_, pad_h_, pad_w_, bottom_diff, this->approx_);
      break;
    case PoolingParameter_PoolMethod_STOCHASTIC:
      // NOLINT_NEXT_LINE(whitespace/operators)
      StoPoolBackward<Dtype>
          <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
              count, rand_idx_.gpu_data(), top_diff, top[0]->num(), channels_,
              height_, width_, pooled_height_, pooled_width_, kernel_h_,
...
INSTANTIATE_LAYER_GPU_FUNCS(PoolingFloatLayer);
